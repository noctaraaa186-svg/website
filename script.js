...
function startLoveLoading() {
  ...
  loadingInterval = setInterval(function() {
    ...
    if (loadingProgress >= 100) {
      ...
      // Setelah loading selesai → tampilkan play gate dulu (sebelum polaroid)
      setTimeout(function() {
...
// ===================================================================
// PLAY GATE — muncul setelah loading session selesai.
// Lagu baru mulai diputar begitu tombol ini ditekan (mengikuti
// kebijakan autoplay browser yang butuh interaksi user), lalu
// transisi polaroid pembuka langsung berjalan bersamaan.
// ===================================================================
function showPlayGate() {
  var gate = document.getElementById('play-gate-overlay');
  if (gate) {
...
function startExperience() {
  ...
  // Lagu mulai bersamaan dengan transisi polaroid
  ...
  if (gate) {
    ...
    showPolaroidTransition(function() { transitionToMain(); });
  }
}
...
window.addEventListener('load', function() {
  spawnHeartCanvas('love-bg-canvas');
  spawnHeartCanvas('songs-bg-canvas');
  spawnHeartCanvas('montage-bg-canvas');
  spawnHeartCanvas('messages-bg-canvas');
  spawnHeartCanvas('closing-bg-canvas');
  // Loading bar jalan dulu, play gate muncul setelah 100%
  startLoveLoading();
});

// ===== HEART CANVAS BACKGROUND (love splash, dipakai di beberapa halaman) =====
function spawnHeartCanvas(canvasId) {
  var canvas = document.getElementById(canvasId || 'love-bg-canvas');
  if (!canvas) return;
  var ctx = canvas.getContext('2d');
